...
function addInCartHandler(product) {
    return () => {
        let foundProduct = productsInCart.find(
            (item) => item.id === product.id,
        );
        const index = productsInCart.findIndex(
            (item) => item.id === product.id,
        );
        count = foundProduct ? foundProduct.count : 1;
...
function saveProductsToLS() {
    localStorage.setItem(PRODUCTS_LS, JSON.stringify(productsInCart));
}

function getProductsFromLS() {
    const productsFromLS = localStorage.getItem(PRODUCTS_LS);
    if (productsFromLS) {
        productsInCart = JSON.parse(productsFromLS);
    }
}
...
function renderCart() {
    ...
    for (let i = 0; i < productsInCart.length; i++) {
        ...
        const productDecreaseButton = document.createElement('button');
        productDecreaseButton.className = 'product-decrease';
        productDecreaseButton.addEventListener(
            'click',
            removeInCartHandler(product.id),
        );

        productInformationAndActions.append(
            productPrice,
            productQuanity,
